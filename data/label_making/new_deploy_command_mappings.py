...
# make this three separate commands, increase (with num), decrease (with num), set (with num)
                elif command_intent == "adjust_volume":
                    # generate embedding for command_tags
                    if command_tags[0] == "increase":
                        dbusMethods.systemVolumeCommand("Inc_sys_vol", command_tags["stub B-volume_increment"])
                    elif command_tags[0] == "decrease":
                        dbusMethods.systemVolumeCommand("Dec_sys_vol", command_tags["stub B-volume_increment"])
                    else:
                        dbusMethods.systemVolumeCommand("Set_sys_vol", command_tags["stub B-volume_increment"])
...
# add show vs open
                elif command_intent == "open_app":
                    # todo map command tag app name to closest app name
                    app_names = ["stub todo"]
                    if command_tags["stub B-app_name through I-app_name"] in app_names: #could be similarity of names
                        dbusMethods.appOpenCloseCommands("stub B-app_name through I-app_name", True, 2)
