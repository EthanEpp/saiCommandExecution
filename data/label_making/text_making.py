...
def tag_sentence(sentence):
    ...
    for token in tokens:
        ...
        if token == '=':
            if in_equals_quotes:
                in_equals_quotes = False  # Closing double quote
            else:
                in_equals_quotes = True
                start_equals_quote = True  # Starting new text content
            continue

        if token.strip():  # Avoiding whitespace
            if in_single_quotes:
                if start_single_quote:
                    output_labels.append('B-contact_name')
                    start_single_quote = False
                else:
                    output_labels.append('I-contact_name')
            elif in_double_quotes:
                if start_double_quote:
                    output_labels.append('B-text_content')
                    start_double_quote = False
                else:
                    output_labels.append('I-text_content')
            elif in_equals_quotes:
                if start_equals_quote:
...
start_timer = [
    "start a 'fried eggs' timer for \"10 minutes\" and make it =yellow=",
    "set quick =green= timer \"5 minutes\" name it 'snack break'",
    "initiate up timer for \"a hour\" called 'relaxation'",
    "begin 'lunch time' timer colored =red= for \"thirty minutes\"",
    "put a 'exercise' timer for \"15 minutes\" in =blue=",
    "create 'meeting' timer color is =purple= and set for \"1 hour\"",
    "start the 'study session' timer \"2 hours\" with =orange=",
    "make a timer of \"ten minutes\" call it 'coffee break' and color =brown=",
    "please start 'yoga' timer for \"half an hour\" make it =pink=",
    "begin with 'game time' timer in =black= for \"45 minute\"",
    "set timer name 'homework' for \"one and half hour\" colored =teal=",
    "initiate 'dinner prep' timer \"40 minutes\" and color it =blue=",
    "make a 'meditation' timer with \"60 min\" color it =gray=",
    "put up 'breakfast' timer \"twenty minutes\" with =white=",
    "start up 'project' timer \"3 hours\" set it to =yellow=",
    "please make a 'tea time' timer for \"25 mins\" with color =green=",
    "set 'walking' timer with =red= for \"30 minute\"",
    "begin the timer name is 'relax' for \"one hour\" make it =purple=",
    "create the 'home clean' timer for \"45 minutes\" in =light blue=",
    "start 'reading' timer with \"90 minutes\" and color is =cyan=",
    "create a 'study break' timer in \"15 min\" with =blue= color",
    "start 'laundry' timer in =orange= for \"40 mins\"",
    "set 'workout' timer for \"1 hour\" with the =purple= color",
    "please start 'dinner prep' timer \"20 min\" and color it =brown=",
    "make a 'project time' timer \"50 minutes\" and set color =pink=",
    "begin 'exercise' timer for \"half hour\" and color it =blue=",
    "set 'movie' timer \"2 hours\" colored =dark green=",
    "start 'meeting' timer \"60 min\" with =grey= color",
    "create 'meditation' timer \"45 minutes\" make it =white=",
    "begin a =black= 'bedtime' timer for \"ten minutes\"",
    "set a =yellow= 'playtime' timer to \"30 mins\"",
    "make 'shower' timer for \"twenty minutes\" with =red=",
    "start 'study time' timer for \"1 hr\" and make it =purple=",
    "create 'cleaning' timer for \"35 minutes\" with =pink=",
    "set 'nap time' timer \"20 mins\" and color =brown=",
    "begin 'focus' timer of \"25 minutes\" with the =white= color",
    "start 'lunch' timer in \"15 mins\" and set it to =green=",
    "create a 'reading' timer \"an hour\" in =blue= color",
    "begin 'work' timer \"50 minutes\" set to =orange=",
    "set up a =yellow= 'homework' timer for \"40 min\"",
    "please create 'cooking' timer \"30 minutes\" with =dark blue= color",
    "make 'exercise time' timer for \"45 mins\" with =black=",
    "start 'break time' timer \"ten minutes\" and color =red="
]

start_unique_timer = [
    "begin 'timer1' \"20 seconds\"",
    "create 'cooking2' timer \"45 minutes\" make it =white=",
    "\"seventeen minutes\" timer is 'blender two'",
    "initiate =green= \"five minute\" timer and name it 'second patient'",
    "\"two hour\" start named 'patient gap2' and color is =white=",
    "start a timer named 'doctor two' for \"five minutes\" and make it =grey=",
    "begin 'checkup' timer for \"thirty minute\" and set =blue=",
    "create 'operation3' timer for \"50 second\" make it =red=",
    "\"ten minute\" timer named 'break time1' in color =yellow=",
    "start a timer for \"an hour\" with name 'shift change' and color =purple=",
    "make 'rounds1' timer \"fifteen minute\" and set color to =pink=",
    "\"three hours\" timer name is 'waiting time' color =orange=",
    "start 'lunch1' timer for \"one and half hour\" with color =light green=",
    "create 'prep time' timer \"twenty-five minutes\" and make it =teal=",
    "begin 'final task' timer \"10 minute\" in =black=",
    "start 'emergency' timer for \"45 minute\" and set to =brown=",
    "\"eight minutes\" timer is 'follow up' with color =grey=",
    "set 'discharge1' timer \"30 seconds\" and color it =blue=",
    "start 'cleaning up' timer with \"one hour\" make it =red=",
    "create 'appointment2' timer for \"50 minutes\" set it to =yellow=",
]

new_joint_commands = [
    "can you explain to me the process of mitosis 'and' solving of 4 times 912",
    "stop speaking 'and' please can you search for good places to stay in japan",
    "since how much time has the timer been on 'and' solve 512*22",
    "start a new timer named is timer1 for seven minutes color is green 'and' timer2 is 5 minutes",
    "tell me what is photosynthesis 'and' how to calculate 23 divide by 5",
    "please search for a restaurant near me 'and' stop the music",
    "what is the distance from earth to sun 'and' what is 99 plus 123",
    "explain the steps for making coffee 'and' start the timer for 10 minutes",
    "how many countries in europe 'and' tell me the time now",
    "start the timer for five minutes 'and' play some relaxing music",
    "search for best price for smartphone 'and' tell me the weather in london",
    "how long the oven has been running 'and' solve 1024 divided by 32",
    "start a blue timer for 3 minutes 'and' set another timer for 10 minutes",
    "what is the answer to 123*12 'and' when is the next eclipse",
    "please stop the current timer 'and' find the meaning of life",
    "explain what is gravity 'and' check temperature of tokyo now",
    "give me recipe for pizza 'and' start a timer for 20 minutes",
    "stop all timers 'and' give me steps to solve quadratic equaton",
    "search for best holiday places in europe 'and' tell me my scedule",
    "start a lunch timer for 45 minutes 'and' stop the music",
    "find recipe for biryani 'and' also what is capital of Australia",
    "tell me what time is it 'and' how to convert 100 USD to INR",
    "please explain how rainbows are made 'and' show me weather of Delhi tomorrow",
    "search how fix laptop 'and' also tell what is result of cricket match today",
    "play music by A.R. Rahman 'and' search for best restaurant nearby",
    "what is the meaning of life 'and' when next festival is hapening in india",
    "stop this timer 'and' make new timer for 10 minutes called study",
    "can you find me a flight from Mumbai to London 'and' tell the best hotel in London",
    "show me how to make dosa 'and' when will be my next reminder",
    "start new countdown for 20 minutes 'and' also tell me how to cook pasta",
    "explain me how to write a email 'and' find popular tourist spots in Goa",
    "tell me when Independence day of India is 'and' solve 56 divided by 8",
    "check weather of Bangalore 'and' give directions to Taj Mahal",
    "please search for best laptop for student 'and' tell me exchange rate of euro to INR",
    "make a new task named homework for 1 hour 'and' start another for 30 minutes name it exercise",
    "set a timer for 15 minutes 'and' also explains how airplanes fly"
    "tell me what is population of brazil 'and' how tall is the eiffel tower",
    "stop the music 'and' play me news on radio",
    "explain how the earth rotate 'and' what is distance to sun",
    "search for best restaurant near me 'and' tell me direction to the park",
    "start study timer for 2 hours color blue 'and' make a break timer for 10 minutes in yellow",
    "what is capital of france 'and' what temperature is it today",
    "how to bake a cake 'and' when is next holiday in usa",
    "search for bus schedule 'and' tell me where the nearest station is",
    "find me some recipes for lunch 'and' give me instructions for cleaning kitchen",
    "create a shopping list with eggs and milk 'and' how do I boil eggs",
    "tell me how long to cook pasta 'and' can you set a 10 minute timer with color is red",
    "search for movie time of Avatar 'and' how do I get to cinema",
    "what is time now in london 'and' how many days until new year",
    "explain steps to change tire on car 'and' show me video on fixing brakes",
    "find recipe for chicken curry 'and' set diner timer for 45 minutes with orange",
    "when does the sun set 'and' tell me weather for tomorrow",
]

open_enlarge = [
    "open 'movie' app in maximized mode",
    "please open 'dicome analyzer' maxmimum mode",
    "start maximized 'zoom' app",
    "please open 'printer' applications is in enlarged window",
    "can you open 'phone' app is in the full screen view",
    "open 'calendar' in big window please",
    "make 'email' app full screen open it",
    "start 'music' app in the largest mode",
    "can you start 'weather' app with full window",
    "open 'calculator' app in is full size screen",
    "please put 'notes' app in maximized mode",
    "begin 'camera' app in bigger mode please",
    "can you open 'games' app in a enlarged screen",
    "please start 'browser' app in full size window",
    "put 'dictionary' app in maxmized view now",
    "start 'gallery' app open is in large screen",
    "could you open 'clock' app in full view screen",
    "make open 'messaging' app is biggest mode",
    "please open 'file manager' app in bigger window mode",
    "start 'video player' app is in full screen view now"
]


sentence_short = [
    "message 'user2' say \"yes\"",
    "text 'boss' and say \"room ready\"",
    "message \"hi\" to 'bruce smith'",
    "send message \"it s starting \" to 'user'",
    "message 'mkbhd' say \"wallpaper too expensive\"",
    "send the message \"hello\" to 'mom'",
    "text 'dad' say \"I arrive soon\"",
    "message 'john' with \"meeting cancel\"",
    "send \"please come\" message to 'sarah'",
    "message 'doctor' with \"i m late\"",
    "text 'boss' and say \"finish the report\"",
    "send message \"not feeling well\" to 'manager'",
    "message 'alex' say \"i am outside now\"",
    "text to 'teacher' say \"homework finish\"",
    "send \"what time meet?\" message to 'jason'",
    "message 'friend2' say \"let s go\"",
    "text 'cousin' with \"bring food\"",
    "message 'client5' \"meeting start soon\"",
    "send \"i am ready\" to 'supervisor'",
    "message 'neighbor2' say \"loud music\"",
]



make_data(sentence_short, "send_text", "output.txt")
