...
dummy_input_ids = torch.randint(0, 30522, (batch_size, seq_length))  # Adjust vocab size if needed
dummy_attention_mask = torch.ones((batch_size, seq_length), dtype=torch.int64)
dummy_token_type_ids = torch.zeros((batch_size, seq_length), dtype=torch.int64)

# Export to ONNX with fixed dimensions
onnx_path = "bert_layer_fixed.onnx"
torch.onnx.export(
    bert_layer,
    (dummy_input_ids, dummy_attention_mask, dummy_token_type_ids),
    onnx_path,
...
print(f"Fixed-dimension ONNX model saved to {onnx_path}")
